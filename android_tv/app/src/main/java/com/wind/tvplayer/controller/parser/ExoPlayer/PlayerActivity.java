...
import com.wind.tvplayer.controller.parser.ExoPlayer.Sample.UriSample;
import com.wind.tvplayer.controller.parser.VideoAnalyzer;
...
public class PlayerActivity extends AppCompatActivity
  ...
  private VideoAnalyzer videoAnalyzer = new VideoAnalyzer();
  private PlayMovie selectedPlayMovie;
  private Sample playSample = null;

  private class ShowSpinnerTask extends AsyncTask<Void, Void, Void> {
    @Override
    protected void onPreExecute() {
      getFragmentManager().beginTransaction().add(R.id.player_view, mSinnerFragment).commit();
    }

    @Override
    protected Void doInBackground(Void... params) {
      // 前置動作
      videoAnalyzer.doParser(selectedPlayMovie.getVideo_url());
      return null;
    }
    ...
    protected void onPostExecute(Void aVoid) {
      ...
      // 收尾動作
      if (videoAnalyzer.getVideoLink().compareTo("") != 0 && player == null) {
        playSample = (Sample) new Sample.UriSample(
          "Apple 16x9 basic stream",
          Uri.parse(videoAnalyzer.getVideoLink()),
          null,
          false,
          null,
          null,
        ...
        // Set Cookie
        dataSourceFactory = ((DemoApplication) getApplication()).buildDataSourceFactory(videoAnalyzer.getCookieMap());
        initializePlayer();
        if (playerView != null) {
          playerView.onResume();
        }
      } else {
        Toast.makeText(getApplicationContext(), "影音解析失敗", Toast.LENGTH_LONG).show();
      }
    }
  }
...
//      }
  ...
  private void initializePlayer() {
    if (player == null) {
      ...
      if (ShareData.getInstance().debugMode) {
...
//    }
    ...
    UriSample[] samples =
            playSample instanceof Sample.PlaylistSample
            ? ((Sample.PlaylistSample) playSample).children
            : new UriSample[] {(UriSample) playSample};
